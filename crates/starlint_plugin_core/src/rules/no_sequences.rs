...
impl LintRule for NoSequences {
    ...
    fn run_on_types(&self) -> Option<&'static [AstNodeType]> {
        Some(&[AstNodeType::SequenceExpression])
    }

    fn run(&self, _node_id: NodeId, node: &AstNode, ctx: &mut LintContext<'_>) {
        let AstNode::SequenceExpression(seq) = node else {
            return;
        };

        // Fix: replace sequence with the last expression (the value of a sequence)
        let fix = seq.expressions.last().and_then(|&last_id| {
            let last_span = ctx.node(last_id)?.span();
            let replacement = ctx
                .source_text()
                .get(
