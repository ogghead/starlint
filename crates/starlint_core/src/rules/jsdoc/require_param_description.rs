...
impl NativeRule for RequireParamDescription {
                    ...
                    if let Some(tag_rest) = trimmed.strip_prefix("@param") {
                        ...
                        if description.trim().is_empty() {
                            ...
                            ctx.report_warning(
                                ...
                                &format!("`@param {param_name}` is missing a description"),
...
mod tests {
    ...
    fn lint(source: &str) -> Vec<starlint_plugin_sdk::diagnostic::Diagnostic> {
        ...
        if let Ok(parsed) = parse_file(&allocator, source, Path::new("test.ts")) {
            let rules: Vec<Box<dyn NativeRule>> = vec![Box::new(RequireParamDescription)];
