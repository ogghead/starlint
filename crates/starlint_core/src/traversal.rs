...
// ---------------------------------------------------------------------------
// AstTree-based dispatch (for LintRule)
// ---------------------------------------------------------------------------
...
///
/// Built once from rule interest declarations. This is the `LintRule` analogue
/// of [`DispatchTable`] (which is for `NativeRule`).
pub(crate) struct LintDispatchTable {
...
///
/// Returns diagnostics from all rules combined.
pub(crate) fn traverse_ast_tree(
    tree: &AstTree,
    rules: &[Box<dyn LintRule>],
